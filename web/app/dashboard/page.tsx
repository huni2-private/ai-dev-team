// Design Ref: §5.1 Dashboard (/dashboard) — 프로젝트 목록 + SDLC 상태
...
export default async function DashboardPage() {
  ...
  if (!bkend.isConfigured) {
    error =
      "Firebase 환경 변수가 설정되지 않았습니다. .env.local에 FIREBASE_PROJECT_ID, FIREBASE_CLIENT_EMAIL, FIREBASE_PRIVATE_KEY를 설정하세요.";
  } else {
    try {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-display-md text-ink">대시보드</h1>
      ...
      {error && (
        <div className="mb-6 rounded-md bg-amber-50 border border-amber-200 px-4 py-3 text-caption text-amber-800">
          <strong>Firebase 연결 필요:</strong> {error}
        </div>
      )}

      {!error && projects.length === 0 && (
        <div className="rounded-lg border border-dashed border-hairline p-16 text-center">
          <p className="text-ink-muted-48 text-caption">
